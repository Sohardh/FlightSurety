...
contract("Flight Surety Tests", async (accounts) => {
  ...
  describe("(airline) test airline functionality", function () {
    ...
    it("(airline) first airline is registered when contract is deployed", async () => {
      let airlineName = await config.flightSuretyData.getAirlineName(
      ...
      assert.equal(
        ...
        "First airline is not registered when contract is deployed"
      );
    });
    ...
    it("(airline) cannot register an airline using registerAirline() if it is not funded", async () => {
      try {
        await config.flightSuretyApp.registerAirline(
          "Indigo Airline",
          airline2,
          { from: config.firstAirlineAddress }
        );
      } catch (e) {
        //console.log(e);
      }
    ...
    it("(airline) airline needs to be funded with 10 ether", async () => {
      ...
      try {
        await config.flightSuretyApp.fundAirline({
          from: config.firstAirlineAddress,
          value: AIRLINE_FUNDING_VALUE_LOWER,
          gasPrice: 0,
        });
      } catch (e) {
        //console.log(e);
      ...
      assert.equal(
        reverted,
        true,
        "Airline cannot be funded with less than 10 ether"
      );
    });
    ...
    it("(airline) airline does participate in contract when it is funded", async () => {
      ...
      assert.equal(
        result,
        true,
        "Airline should be able to register another airline if it has been funded"
      );
    });
    ...
    it("(airline) can register up to 4 airlines", async () => {
      ...
      assert.equal(
        result,
        true,
        "Registering the third airline should be possible"
      );

      try {
        await config.flightSuretyApp.registerAirline(
          "Air Cananda Airlines",
          airline4,
          { from: config.firstAirlineAddress }
        );
      } catch (e) {
      ...
      assert.equal(
        result,
        true,
        "Registering the fourth airline should be possible"
      );
    });
    ...
    it("(airline) single airline cannot register 5th airline on its own", async () => {
      ...
      assert.equal(
        result,
        false,
        "Registering the fifth airline should not be possible"
      );
    });
    ...
    it("(airline) 5th airline requires multi-party consensus of 50% of registered airlines", async () => {
      ...
      assert.equal(
        result,
        true,
        "Registering the fifth airline should be possible"
      );
    });
  });
  ...
  describe("(flight) test flight functionality", function () {
    it("(flight) can register new flight", async () => {
      ...
      try {
        ...
        await config.flightSuretyApp.registerFlight(
          ...
          flight2.from,
          flight2.timestamp,
          { from: flight2.airline }
        );
      } catch (e) {
        console.log(e);
      }
      result = await config.flightSuretyData.isFlight.call(
        flight1.airline,
        flight1.flight,
        flight1.timestamp
      );
    ...
    it("(flight) cannot register a flight  more than once", async () => {
      ...
      assert.equal(
        reverted,
        true,
        "Airline cannot register a flight more than once"
      );
    });
    ...
    it("(flight) cannot register a flight if the airline is not funded", async () => {
      ...
      assert.equal(
        reverted,
        true,
        "Airline cannot register a flight if it is not funded"
      );
    });
  });
  ...
  describe("(passenger) test passenger functionality", function () {
    it("(passenger) cannot buy insurance for non-registered flight", async () => {
      ...
      try {
        await config.flightSuretyApp.buyInsurance(
          ...
          flight3.flight,
          flight3.timestamp,
          { from: passenger1, value: PASSENGER_INSURANCE_VALUE, gasPrice: 0 }
        );
      } catch (e) {
        //console.log(e);
        reverted = true;
    ...
    it("(passenger) cannot buy insurance without funds", async () => {
      ...
      try {
        await config.flightSuretyApp.buyInsurance(
          ...
          flight2.flight,
          flight2.timestamp,
          { from: passenger1, value: 0, gasPrice: 0 }
        );
      } catch (e) {
        //console.log(e);
        reverted = true;
    ...
    it("(passenger) cannot buy insurance above insurance limit", async () => {
      ...
      try {
        await config.flightSuretyApp.buyInsurance(
          ...
          {
            from: passenger1,
            value: PASSENGER_INSURANCE_VALUE_1 + 1,
            gasPrice: 0,
          }
        );
      } catch (e) {
    ...
    it("(passenger) can buy insurance", async () => {
      ...
      try {
        await config.flightSuretyApp.buyInsurance(
          ...
          flight2.flight,
          flight2.timestamp,
          { from: passenger1, value: PASSENGER_INSURANCE_VALUE_1, gasPrice: 0 }
        );
      } catch (e) {
        console.log(e);
      }
      result = await config.flightSuretyData.isInsured.call(
        passenger1,
        flight2.airline,
        flight2.flight,
        flight2.timestamp
    ...
    it("(passenger) cannot buy insurance for the same flight twice", async () => {
      ...
      assert.equal(
        reverted,
        true,
        "Passenger cannot buy insurance for the same flight twice"
      );
    });
    ...
    it("(passenger) more than one passenger can register for the same flight", async () => {
      ...
      try {
        await config.flightSuretyApp.buyInsurance(
          ...
          flight2.flight,
          flight2.timestamp,
          { from: passenger2, value: PASSENGER_INSURANCE_VALUE_2, gasPrice: 0 }
        );
      } catch (e) {
        console.log(e);
      }
    ...
    it("(oracles) can register oracles", async () => {
      ...
      for (let a = 1; a < TEST_ORACLES_COUNT; a++) {
        await config.flightSuretyApp.registerOracle({
          from: accounts[a + ORACLES_OFFSET],
          value: fee,
        });
  ...
  describe("(insurance) test passenger functionality", function () {
    it("(insurance) insured amount credited is multiplied by the configured multiplier", async () => {
      let amount1 = await config.flightSuretyData.getPendingPaymentAmount(
        passenger1
      );
      let amount2 = await config.flightSuretyData.getPendingPaymentAmount(
        passenger2
      );
      let multiplier = 1.5;
      assert.equal(
        amount1,
        PASSENGER_INSURANCE_VALUE_1 * 1.5,
        "Insurance amount not as expected"
      );
      assert.equal(
        amount2,
        PASSENGER_INSURANCE_VALUE_2 * 1.5,
        "Insurance amount not as expected"
      );
    });
    ...
    it("(insurance) can withdraw amount", async () => {
      ...
      assert.equal(
        balanceAfterPay1 - balanceBeforePay1,
        amount1,
        "Cannot withdraw insurance from account"
      );
      assert.equal(
        balanceAfterPay2 - balanceBeforePay2,
        amount2,
        "Cannot withdraw insurance from account"
      );
    });
